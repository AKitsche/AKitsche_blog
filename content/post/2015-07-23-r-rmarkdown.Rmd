...
Unfortunatly we probably get an error: *Error in curl::curl_fetch_memory(url, handle = handle) : Couldn't resolve host name*, because the Continental network environment uses HTTP Proxy. To pick out the proxy setting we can use the `ie_proxy_info()` function from the [curl](https://cran.r-project.org/web/packages/curl/index.html) R add-on package.:
